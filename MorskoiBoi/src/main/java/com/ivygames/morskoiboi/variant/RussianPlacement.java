...
public class RussianPlacement implements PlacementAlgorithm {

    private final Random mRandom;

    public RussianPlacement() {
    ...
    public boolean place(Ship ship, Board board) {
        ...
        while (!cells.isEmpty()) {
            ...
            if (found) {
                found = RussianPlacement.isPlaceEmpty(ship, board, i, j);
                if (found) {
    ...
    private Ship.Orientation calcRandomOrientaiton() {
        return mRandom.nextInt(2) == 1 ? Ship.Orientation.HORIZONTAL : Ship.Orientation.VERTICAL;
    }
